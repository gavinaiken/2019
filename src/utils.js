...
async function getInput(day, year = 2019) {
    await mkCacheDir();
...
function splitNewlinesToArrayOfNumber(input) {
    return input.split(/[\r\n]/).map(i => Number(i));
}

function splitCommasToArrayOfNumber(input) {
    return input.split(/\s*,\s*/).map(i => Number(i));
}

module.exports = { getInput, splitNewlinesToArrayOfNumber, splitCommasToArrayOfNumber };
